...
#     def print(self):
#         print "--"
# print "positive: ", self.positive, ". percentage: ", self.positive / \
...
# assumes captures low point and high point
for i in range(1, spyClose.shape[0]):
    vixIndex = min(i-1, len(vixVal) - 1)
    if(spyChangeHigh[i-1] >= vixVal[vixIndex]*stdMultiple/sqrtDays):
        num = int(prevMoney / spyHigh[i-1])
        money[i] = prevMoney + (spyHigh[i-1] - spyClose[i])*num
        prevMoney = money[i]
    ...
    elif (spyChangeLow[i-1] <= -vixVal[vixIndex]*stdMultiple/sqrtDays):
        num = int(prevMoney / spyLow[i-1])
        money[i] = prevMoney + (spyClose[i] - spyLow[i-1])*num
        prevMoney = money[i]
        if spyChangePercent[i] >= 0.0:
            holdTilClose.positive += 1.0
        else:
...
print "high:", spyClose[-1]*(1.0 + vixVal[-1]*stdMultiple/sqrtDays/100.0)[0]
